		...
		parallel_exec(nbInnerCells, [&](const size_t& jInnerCells)
		{
			...
			RealArray1D<2> reduction0({0.0, 0.0});
			{
				...
				for (size_t rNodesOfCellJ=0; rNodesOfCellJ<nbNodesOfCellJ; rNodesOfCellJ++)
				{
					...
					reduction0 = schema1freefuncs::sumR1(reduction0, X[rNodes]);
				}
			}
 ...
 * Out variables: t_n0
 */
void Schema1::initTime() noexcept
{
	t_n0 = 0.0;
}
 ...
 * Out variables: Uini
 */
void Schema1::initUini() noexcept
{
	{
 ...
 */
void Schema1::updateDij() noexcept
{
 ...
 */
void Schema1::updateHcalc() noexcept
{
	{
		const auto innerCells(mesh->getInnerCells());
		const size_t nbInnerCells(innerCells.size());
 ...
 */
void Schema1::initUcalc() noexcept
{
 ...
 */
void Schema1::updateHinner() noexcept
{
	{
		const auto innerCells(mesh->getInnerCells());
		const size_t nbInnerCells(innerCells.size());
 ...
 * Out variables: H_nplus1
 */
void Schema1::updateHouter() noexcept
{
	{
		const auto topCells(mesh->getTopCells());
		const size_t nbTopCells(topCells.size());
 ...
 */
void Schema1::updateUcalc() noexcept
{
	{
		const auto innerCells(mesh->getInnerCells());
		const size_t nbInnerCells(innerCells.size());
